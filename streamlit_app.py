...
# Trên Streamlit Cloud không có file .env — đổ st.secrets vào env để os.getenv đọc được
# (OPENAI_API_KEY, EMBEDDING_PROVIDER, …). Cấu hình ở: app → Settings → Secrets.
try:
    for _k, _v in st.secrets.items():
        os.environ.setdefault(_k, str(_v))
except Exception:  # noqa: BLE001 — không có secrets thì bỏ qua
    pass

from ai_sre.embeddings import get_embedder
from ai_sre.ingest import build_index
...
def load_store_and_embedder(index_dir: str):
    ...
    index/ là build artifact (gitignore) nên khi deploy (Streamlit Cloud) sẽ không có
    → tự dựng từ data/ ở lần chạy đầu. build_index dùng cùng embedder với query.
    """
    if not Path(index_dir, "meta.json").exists():
